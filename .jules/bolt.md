...
## 2026-05-11 - O(N) Reductions in Profile Rendering\n**Learning:** The user profile rendering logic previously filtered the global `reviews` array with `reviews.filter(r => r.author === profileUsername)` to find the current user's reviews, causing O(N) operations whenever the profile modal was opened.\n**Action:** Replaced the array filter with a global Map `reviewsByAuthor`, populated during the `updateReviewsMap()` setup phase, enabling rapid O(1) lookups via `reviewsByAuthor.get(profileUsername)`. Keep cache updates incremental rather than clearing Maps entirely.
