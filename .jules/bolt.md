...
## 2024-05-29 - Prevent unnecessary memory allocation and GC overhead during rendering and sorting\n**Learning:** The application was unconditionally copying an array `[...releases]` before filtering, causing an unnecessary O(N) allocation, and the sort comparator created new empty arrays `[]` when `map.get()` was falsy, causing high memory usage and garbage collection overhead during repeated rendering/sorting operations.\n**Action:** Avoid intermediate array copies by executing `.filter()` directly when appropriate or copying only when needed. Use optional chaining `map.get(id)?.length || 0` rather than allocating empty fallback arrays to prevent excessive GC overhead during $O(N \log N)$ sort iterations.\n
