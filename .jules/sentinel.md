...
## 2026-05-11 - Prevent path traversal in dynamically built API request URLs
**Vulnerability:** Path parameters like `releaseToDelete` and `pendingReviewDelete` in `fetch` DELETE requests were not sanitized using `encodeURIComponent()`.
**Learning:** Unsanitized path parameters can lead to path traversal vulnerabilities if they contain characters like `/` or `..`.
**Prevention:** Always sanitize variables used in dynamically built API request URLs (e.g., appending IDs to paths) with `encodeURIComponent()`.
