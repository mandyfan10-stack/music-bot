...
## 2024-05-31 - Prevent Path Traversal in API Requests\n**Vulnerability:** When dynamically building API request URLs (e.g., appending IDs to paths for a DELETE request in `index.html`), variables like `pendingReviewDelete` and `releaseToDelete` were not sanitized.\n**Learning:** Unsanitized path parameters can lead to path traversal vulnerabilities if they contain characters like `/` or `..`.\n**Prevention:** Always sanitize dynamic path parameters using `encodeURIComponent()` before appending them to a URL to prevent path traversal.
