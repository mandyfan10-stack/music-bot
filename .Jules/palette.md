...
## 2026-05-08 - Modal Keyboard Accessibility
**Learning:** Custom modals (e.g. `slide-up-modal` implementations) dynamically generated and controlled via JS must explicitly handle the `Escape` key to fulfill keyboard accessibility criteria. Relying only on click-outside `.modal-overlay` or visual close buttons creates friction for screen reader and keyboard-only users.
**Action:** When creating or managing modals, track their visibility state (e.g., using a LIFO array like `activeModals`) and attach a global `keydown` listener to intercept the `Escape` key to close the topmost open modal.
