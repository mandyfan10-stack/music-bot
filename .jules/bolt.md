...
## 2024-05-29 - Unnecessary Array Allocations in Filters and Comparators
**Learning:** Creating intermediate array copies (e.g., `[...arr]`) before using `.filter()` is redundant and causes unnecessary memory allocation since `.filter()` already returns a new array. Additionally, allocating empty fallback arrays (e.g., `|| []`) inside tight loops like sort comparators generates massive GC overhead.
**Action:** Avoid copying arrays before filtering. In sort comparators and tight loops, prefer optional chaining (e.g., `map.get(id)?.length || 0`) instead of fallback arrays to prevent memory thrashing.
