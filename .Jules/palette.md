...
## 2026-04-29 - Modal Keyboard Accessibility
**Learning:** Native-feeling keyboard accessibility for custom modals in vanilla JS requires explicit tracking of open modals via a stack (e.g., `activeModals = []`) and a global `keydown` event listener for the `Escape` key that safely pops from the stack.
**Action:** Always implement stack-based `Escape` key handling when building custom modal components to ensure assistive technologies and keyboard users can reliably dismiss layered dialogs.
