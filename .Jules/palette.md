...
## 2026-05-05 - Modal Escape Key Navigation
**Learning:** Custom modal implementations natively missing `Escape` key support severely degrade keyboard navigation experiences for power users and assistive technologies, trapping focus or forcing mouse interactions.
**Action:** Always implement a stack-based tracker (e.g., `activeModals`) and attach a global `keydown` listener to handle `Escape` keypresses, ensuring the topmost/most recently opened custom modal is properly closed.
